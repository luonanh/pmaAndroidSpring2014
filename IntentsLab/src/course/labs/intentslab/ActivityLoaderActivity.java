...
public class ActivityLoaderActivity extends Activity {
	...
	private void startExplicitActivation() {
		...
		Intent i1 = new Intent(ActivityLoaderActivity.this, ExplicitlyLoadedActivity.class);
		
		// TODO - Start an Activity using that intent and the request code defined above
		startActivityForResult(i1, GET_TEXT_REQUEST_CODE);
	}
	...
	private void startImplicitActivation() {
		...
		//Log.i(TAG,"Web Intent Action:" + webIntent.getAction());
		...
		Intent chooserIntent = Intent.createChooser(webIntent, title);	// Create intent to show chooser
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		if (GET_TEXT_REQUEST_CODE == requestCode) {	// Check which request we're responding to
			if (Activity.RESULT_OK == resultCode) {	// Make sure the request was successful
				//Uri text = data.getData();
