...
public class CreateStoryActivity extends StoryActivityBase {
	...
	private void launchSoundIntent() {
		...
		Intent i1 = new Intent(this, SoundRecordActivity.class); // ALDBG follow IntentLab ActivityLoaderActivity.java
		
		// TODO - Use getOutputMediaFile() to create a new 
		// filename for this specific sound file
		if (!Environment.getExternalStorageState().equals(
				Environment.MEDIA_MOUNTED)) { // ALDBG follow ContentLab PlaceViewAdapter.java
			return;
		}
		...
		// ALDBG originally did this, but then follow forum i1.putExtra(SoundRecordActivity.EXTRA_OUTPUT, newFile);
		i1.putExtra(SoundRecordActivity.EXTRA_OUTPUT, newFile.getPath());
		
		// TODO - Start a new activity for result, using the new intent and the request
		// code MIC_SOUND_REQUEST
		startActivityForResult(i1, CreateStoryActivity.MIC_SOUND_REQUEST);
	...
	private void launchCameraIntent() {
		...
		fragment.imagePath = newFileUri;
		
		
		// TODO - Start a new activity for result, using the new intent and the request
		// code CAMERA_PIC_REQUEST
		startActivityForResult(i1, CreateStoryActivity.CAMERA_PIC_REQUEST);
		// ALDBG should  we finish activity here finish();
	}
	...
	private void launchVideoCameraIntent() {
		...
		// TODO - Add the filename to the Intent as an extra. Use the Intent-extra name
		// from the MediaStore class, EXTRA_OUTPUT
		i1.putExtra(MediaStore.EXTRA_OUTPUT, newFile.getPath());
		...
		i1.putExtra(MediaStore.EXTRA_VIDEO_QUALITY, 1);
		
		// TODO - Start a new activity for result, using the new intent and the request
		// code CAMERA_VIDEO_REQUEST
		startActivityForResult(i1, CreateStoryActivity.CAMERA_VIDEO_REQUEST);
		// ALDBG should  we finish activity here finish();
	}
